...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivisen_saldon_lataaminen_ei_onnistu(self):
        self.kortti.lataa_rahaa(-5)
        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10 euroa")

    def test_tasarahalla_pystyy_syomaan_edullisesti(self):
        for i in range(4):
            self.kortti.syo_edullisesti()
        self.assertEqual(str(self.kortti), "Kortilla on rahaa 0.0 euroa")

    def test_tasarahalla_pystyy_syomaan_maukkaasti(self):
